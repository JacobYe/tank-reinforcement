...
public class PlayerInteract {
    ...
    Logger LOG_RPC = LoggerFactory.getLogger("RPC");
        ...
        t = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                for (; ; ) {
                    ...
                    try {
                        GameState state = statusQueue.take();
                        LOG_RPC.debug("Send state to " + getAddress() + " : " + PlayerInteract.toString(convert(state)));
                        //LOG_RPC.debug("Send state to " + getAddress() + " : " + Util.toJson(state));
                    ...
                    try {
                        ...
                        LOG_RPC.debug("Recv orders from " + getAddress() + " : " + PlayerInteract.toString(orders));
    ...
    protected boolean verifyPlayerInput(List<TankOrder> tankOrders) {
        if (tankOrders.stream().filter(o -> !tanks.contains(o.getTankId())).count() > 0) {
            tankOrders.stream().filter(o -> !tanks.contains(o.getTankId())).forEach(o -> LOG_RPC.debug(address + " try to control enemy's tank:" + o));
            return false;
        }
        if (tankOrders.stream().map(TankOrder::getTankId).collect(Collectors.toSet()).size() < tankOrders.size()) {
            LOG_RPC.debug(address + " has duplicate orders");
            return false;
        }
        return true;
    }
