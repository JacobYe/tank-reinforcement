...
public class GameStateMachine {
    ...
    private Logger LOG_REPLAY = LoggerFactory.getLogger("REPLAY");
    ...
    // *** START *** my code
    public long[] countScore(String name, GameOptions gameOptions) {
        long[] rst = new long[4];
        Player own = players.get(name);
        Player enemy = players.get(players.keySet().stream().filter(s->!s.equals(name)).findFirst().get());
        rst[1] = own.getTanks().stream().filter(id -> tanks.containsKey(id) && !tanks.get(id).isDestroyed()).count();//防守得分：生存数量
        rst[2] = enemy.getTanks().stream().filter(id -> tanks.containsKey(id) && tanks.get(id).isDestroyed()).count();//进攻得分：消灭数量
        rst[3] = own.getNoOfFlag();//取旗得分: 取旗数量
        rst[0] = rst[1] * gameOptions.getTankScore() + rst[3] * gameOptions.getFlagScore();//总得分
        return rst;
    }
    // *** END   *** my code
    ...
    private void evaluateFireActions(List<TankOrder> orders) {
        ...
        for (TankOrder order : orders) {
            ...
            if (shell != null) {
                LOG_ENGINE.debug("Tank " + order.getTankId() + " fire a new shell :" + shell);
                newShells.add(shell);
            }
        }
    ...
    public Position generateFlag() {
        ...
        LOG_ENGINE.debug("Generate flag at " + flagPos);
    ...
    public boolean gameOvered() {
        ...
        for (Player p : players.values()) {
            if (p.getTanks().stream().noneMatch(id -> tanks.containsKey(id) && !tanks.get(id).isDestroyed())) {
                LOG_ENGINE.debug("Player " + p.getName() + " loses all tanks!");
                result = true;
            }
        }
        return result;
