...
public class GameEngine {
    ...
    private Logger LOG_ENGINE = LoggerFactory.getLogger("ENGINE");

    GameResult result = new GameResult();

    // *** START *** my code
    JudgeResultCallBack callBack;

    public JudgeResultCallBack getCallBack() {
        return callBack;
    }

    public void setCallBack(JudgeResultCallBack callBack) {
        this.callBack = callBack;
    }

    public class JudgeResultCallBack {
        public long[] calc(String name) {
            return stateMachine.countScore(name, gameOptions);
        }
    }
    ...
    void parseArgs(String[] args) {
        ...
        if (noOfTanks > 5) {
            LOG_ENGINE.debug("Max no of tanks is 5");
            System.exit(-1);
        }
        ...
        LOG_ENGINE.debug("Parameters parsed. " + this.gameOptions);
    }
    ...
    void calculateResult(int round) {
        ...
        LOG_ENGINE.debug("Game result: " + Util.toJson(result));
    }

    void reportResult() {
//        String resUrl = env.get("WAR_CALLBACK_URL");
//        LOG_ENGINE.debug("WAR_CALLBACK_URL=" + resUrl);
...
//            LOG_ENGINE.debug(response.toString());
//        } catch (IOException e) {
//            LOG_ENGINE.debug("Failed to send!");
//            e.printStackTrace();
//        }
//        LOG_ENGINE.debug("Result sent.");
    }
    ...
    protected void checkGenerateFlag(int round) {
        if (flagGenerated == false) {
            ...
            if (round > (gameOptions.getMaxRound() / 2 - 1) && stateMachine.getTankList().size() == 2 * gameOptions.getNoOfTanks()) {
                LOG_ENGINE.debug("Start to generate flag.");
                flagGenerated = true;
        ...
        public void run() {
            for (int i = 0; i < 300; i++) {
                try {
                    ...
                    LOG_ENGINE.debug("Failed to connect to " + this.name);
                    e.printStackTrace();
                }
                try {
                    Thread.sleep(1000);
    ...
    private void connectToPlayers() {
        ...
        if (this.clients.size() < 2) {
            ...
            LOG_ENGINE.debug(Util.toJson(result));
            reportResult();
            //do not exit the main thread cause the judge process needs me to alive
            try {
                Thread.sleep(1000 * 60 * 10);
    ...
    private PlayerServer.Client buildPlayerConnection(String addr) throws TTransportException {
        ...
        LOG_ENGINE.debug("Succeed to connect to  " + addr);
        return client;
    }
